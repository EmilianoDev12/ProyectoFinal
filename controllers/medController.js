...
module.exports = {
    ...
    agregarExpediente:function (req, res, next) {
        if(req.session.MUsuario == undefined){
            res.redirect('/isPaciente');
        }else {
            expediente.buscarFich(con, req.body.IdPac, function (err, data) {
                console.log(data);
                const Fich = data[0];
                const IdPac = req.body.IdPac;
                const Tipo = "Siu";
    ...
    cargarExpediente:function (req, res, next) {
        if(req.body.IdPac == undefined){
            res.redirect('/isPaciente');
        }else {
            console.log(req.body);
            req.body.Ano = req.body.Fecha.substring(0,4);
            req.body.Mes = req.body.Fecha.substring(5,7);
            req.body.Dia = req.body.Fecha.substring(8);
            expediente.actualizarFich(con, req.body.IdPac, req.body, function (err) {
                console.log(err);
                res.redirect('/pantallaInicial');
            });
        }
    },
    cargarAntecedentes:function (req, res, next) {
        if(req.body.IdPac == undefined){
            res.redirect('/isPaciente');
        }else {
            console.log(req.body);
